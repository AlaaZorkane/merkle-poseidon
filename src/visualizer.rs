...
impl Visualizer for SparseMerkleTree<Poseidon<Fr>> {
    fn visualize(&self) {
        ...
        visualize_node(&self.root, 0, "".to_string(), true);
    }
}
...
fn visualize_node(
    ...
    match &node_ref.node_type {
        NodeType::Inner(hash) => {
            println!("{}{} (Inner Node: {})", indent, depth, short_fr(hash));
            ...
            if let Some(left) = &node_ref.left {
                visualize_node(left, depth + 1, child_prefix.clone(), false);
            } else {
                println!("{}├── {} (Empty)", child_prefix, depth + 1);
            }

            // Visualize right child
            if let Some(right) = &node_ref.right {
                visualize_node(right, depth + 1, child_prefix, true);
            } else {
                println!("{}└── {} (Empty)", child_prefix, depth + 1);
            }
        }
        NodeType::Leaf(value) => {
            println!("{}{} (Leaf Value: {})", indent, depth, short_fr(value));
        }
    }
}
...
mod tests {
    ...
    fn test_visualization() {
        ...
        // Insert a few values
        let path1 = Fr::from(1u64); // Simple value for path
        let path2 = Fr::from(6u64); // Binary 110, simple way to create different paths
